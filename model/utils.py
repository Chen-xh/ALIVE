"""Runtime, distributed-training, and reproducibility utilities."""
...
def set_seed(seed):
    ...
    参数:
        seed (int): 随机种子值
    """
    # 设置Python哈希种子，影响hash()等函数的随机性
    os.environ['PYTHONHASHSEED'] = str(seed)

    # 设置PyTorch的随机种子
    torch.cuda.manual_seed(seed)  # 设置当前GPU的随机种子
    torch.cuda.manual_seed_all(seed)  # 设置所有GPU的随机种子（多GPU时）
    ...
    # 配置PyTorch的CUDA后端以确保确定性
    torch.backends.cudnn.deterministic = True  # 确保每次运行卷积算法相同
    torch.backends.cudnn.benchmark = False  # 禁用自动寻找最优卷积算法
