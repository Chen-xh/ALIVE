...
def encode_data(model, data_loader, log_step=10, logging=logger.info):
    ...
    for i, data_i in enumerate(data_loader):
        ...
        if len(data_i) == 5:       
            ...
            images, captions, lengths, long_captions, long_lengths, ids, img_ids = data_i
...
def evalrank(
    ...
    # New checkpoints carry base_epochs and use identity_count * captions.
    # Older checkpoints predate that convention and already store the complete
    # evaluation batch in batch_size.
    if hasattr(opt, "base_epochs"):
        evaluation_batch_size = opt.batch_size * opt.captions_per_image
    else:
        evaluation_batch_size = opt.batch_size
    data_loader = image_caption.get_test_loader(
        opt,
        data_path,
        tokenizer,
    ...
    if not fold5:
        ...
        r, rt = i2t(npts, sims, return_ranks=True,mode=opt.dataset)
        ri, rti = t2i(npts, sims, return_ranks=True,mode=opt.dataset)
        ...
        for i in range(5):
            ...
            r, rt0 = i2t(npts, sims, return_ranks=True,mode=opt.dataset)
            ri, rti0 = t2i(npts, sims, return_ranks=True,mode=opt.dataset)
